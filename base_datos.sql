...
-- Datos de ejemplo
INSERT INTO grados (nombre, tipo, orden) VALUES
...
INSERT INTO grupos_descanso (nombre, tipo, color) VALUES
...
INSERT INTO tipos_servicio (nombre, tipo_personal, cantidad_especialistas, cantidad_soldados, requiere_oficial, duracion_horas, prioridad_asignacion) VALUES
...
LEFT JOIN calendario_descansos cd ON 
    cd.id_grupo_descanso = p.id_grupo_descanso 
    AND CURDATE() BETWEEN cd.fecha_inicio AND cd.fecha_fin
WHERE p.activo = TRUE;
...
SELECT 
    ...
    asig.hora_fin,
    asig.estado
FROM asignaciones_servicio asig
...
WHERE asig.fecha_servicio = CURDATE()
ORDER BY ts.nombre, asig.hora_inicio;
...
FROM personal p
LEFT JOIN asignaciones_servicio asig ON 
    p.id_personal = asig.id_personal 
    AND asig.fecha_servicio >= DATE_SUB(CURDATE(), INTERVAL 30 DAY)
    AND asig.estado != 'CANCELADO'
WHERE p.activo = TRUE
GROUP BY p.id_personal, p.nombres, p.apellidos, p.tipo;
...
-- ========================================
CREATE PROCEDURE sp_asignar_servicios_automatico(
    IN p_fecha DATE
)
BEGIN
    DECLARE v_servicio_noche_id INT;
    ...
    SELECT id_tipo_servicio INTO v_servicio_noche_id FROM tipos_servicio WHERE nombre = 'SERVICIO_NOCHE';
    SELECT id_tipo_servicio INTO v_tactico_id FROM tipos_servicio WHERE nombre = 'TACTICO';
    SELECT id_tipo_servicio INTO v_reconocimiento_id FROM tipos_servicio WHERE nombre = 'RECONOCIMIENTO';
    ...
    IF v_error = 0 THEN
        COMMIT;
        SET v_mensaje = CONCAT('✅ Servicios asignados exitosamente para ', DATE_FORMAT(p_fecha, '%d/%m/%Y'));
    ELSE
        SET v_mensaje = '❌ Error al asignar servicios';
    END IF;
...
BEGIN
    ...
    DECLARE cur_disponibles CURSOR FOR
        SELECT id_personal FROM tmp_disponibles ORDER BY prioridad DESC, dias_sin_servicio DESC;
    ...
    SELECT 
        ...
        cantidad_soldados,
        requiere_oficial
    INTO v_cantidad_esp, v_cantidad_sold, v_requiere_oficial
    FROM tipos_servicio 
    WHERE id_tipo_servicio = p_tipo_servicio;
    ...
    IF v_requiere_oficial THEN
        SELECT id_personal INTO v_id_oficial
        FROM personal p
        WHERE p.tipo = 'OFICIAL'
            AND p.activo = TRUE
            AND p.es_encargado = TRUE
            -- No está de descanso (oficiales pueden o no tener grupo)
            AND (p.id_grupo_descanso IS NULL OR NOT EXISTS (
                SELECT 1 FROM calendario_descansos cd
                WHERE cd.id_grupo_descanso = p.id_grupo_descanso
                    AND p_fecha BETWEEN cd.fecha_inicio AND cd.fecha_fin
            ))
            -- No tiene otro servicio asignado
            AND NOT EXISTS (
                SELECT 1 FROM asignaciones_servicio asig
                WHERE asig.id_personal = p.id_personal
    ...
    IF v_cantidad_esp > 0 THEN
        ...
        FROM personal p
        LEFT JOIN historial_rotaciones hr ON 
            hr.id_personal = p.id_personal 
        ...
        ORDER BY dias_sin_servicio DESC, prioridad DESC
        LIMIT v_cantidad_esp;
    END IF;
    ...
    IF v_cantidad_sold > 0 THEN
        ...
        WHERE p.tipo = 'TROPA'
            ...
            AND (
                p_tipo_servicio != (SELECT id_tipo_servicio FROM tipos_servicio WHERE nombre = 'RECONOCIMIENTO')
                OR NOT EXISTS (
                    SELECT 1 FROM asignaciones_servicio asig
                    INNER JOIN tipos_servicio ts ON asig.id_tipo_servicio = ts.id_tipo_servicio
                    WHERE asig.id_personal = p.id_personal
    ...
    read_loop: LOOP
        FETCH cur_disponibles INTO v_id_personal;
        IF done THEN
            LEAVE read_loop;
        END IF;
        
        INSERT INTO asignaciones_servicio (
            id_personal,
            id_tipo_servicio,
...
-- Procedimiento: Obtener personal disponible para un tipo de servicio en una fecha
...
-- Trigger: Actualizar historial después de asignar servicio
...
BEGIN
    ...
    VALUES (NEW.id_personal, NEW.id_tipo_servicio, NEW.fecha_servicio, 0, 0)
